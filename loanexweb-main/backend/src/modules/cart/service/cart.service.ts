...
function resolvePricing(row: CartRow) {
  ...
  if (variant) {
    ...
    const unitPrice = variant.discountPrice == null ? mrp : toNumber(variant.discountPrice);
  ...
  const mrp = toNumber(row.product.price);
  const unitPrice =
    row.product.discountPrice == null ? mrp : toNumber(row.product.discountPrice);
  return {
    mrp,
    unitPrice,
...
function mapItem(row: CartRow) {
  ...
  const available = row.product.isActive && pricing.stock > 0;
...
export class CartService {
  ...
  async addItem(userId: string, input: AddCartItemBody) {
    ...
    } else if (!product.isActive || product.stock <= 0) {
  ...
  async updateItem(userId: string, cartItemId: string, input: UpdateCartItemBody) {
    ...
    if (!existing.product.isActive || pricing.stock <= 0) {
      throw new BadRequestError('Product is out of stock.', { code: 'OUT_OF_STOCK' });
    }
