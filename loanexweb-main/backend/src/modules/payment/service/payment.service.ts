...
function buildSummary(app: {
  ...
  approvedDownPayment: { toString(): string } | null;
  requestedAmount: { toString(): string };
  requestedDownPayment: { toString(): string };
...
export class PaymentService {
  ...
  async verifyPayment(
    ...
    await auditLogService.log({
      ...
      metadata: {
        ...
        estimatedDeliveryDate: result.order.estimatedDeliveryDate?.toISOString() ?? null,
        timestamp: new Date().toISOString(),
      },
    });
  ...
  async getOrderConfirmation(userId: string, orderNumber?: string) {
    const order = orderNumber
      ...
      : await paymentRepository.findLatestApplicationForUser(userId).then(async (app) => {
    ...
    const summary = buildSummary(order.application);
    ...
    return {
      ...
      productId: order.productId,
      productName: order.application.productName,
