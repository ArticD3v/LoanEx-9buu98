...
export interface CreateCheckoutResponse {
  ...
  summary: Pick<CheckoutSummary, 'product' | 'quantity' | 'pricing'>;
  redirectPath: string;
...
export class CheckoutApiService {
  ...
  getSummary(productId: string, quantity = 1, variantId?: string): Observable<CheckoutSummary> {
    let params = new HttpParams().set('quantity', String(quantity));
    if (variantId) {
      params = params.set('variantId', variantId);
    }
    return this.wrap(
      this.http.get<ApiSuccess<CheckoutSummary>>(`${this.baseUrl}/${productId}`, { params }),
    );
  }
