...
export function getMockExistingSkus(additional: string[] = []): string[] {
  const fromProducts = MOCK_PRODUCTS.map((p) => p.sku);
  const demo = ['TN-X1-256-001', 'TN-X1-256-002'];
  return [...new Set([...fromProducts, ...demo, ...additional.filter(Boolean)])];
}
