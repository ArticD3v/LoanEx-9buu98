  ...
  return (
    <View style={styles.container}>
      ...
      <ReviewCard title="Basic Information">
        ...
        <ReviewRow label="Colour / Size / Variant" value={formData.colourSizeVariant || '—'} />
        <ReviewRow label="Product Video URL" value={formData.productVideoUrl || '—'} />
      ...
      <ReviewCard title="Images">
        <View style={styles.imageRow}>
          {formData.primaryImage ? (
            <Image source={{ uri: formData.primaryImage }} style={styles.thumb} />
          ) : (
...
function ReviewRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <View style={styles.row}>
      ...
      <Text style={[styles.value, highlight && styles.highlight]} numberOfLines={2}>{value}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  thumb: { width: 60, height: 60, borderRadius: 8, resizeMode: 'cover' },
});
