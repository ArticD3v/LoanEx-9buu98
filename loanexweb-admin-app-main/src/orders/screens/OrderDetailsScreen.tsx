...
export function OrderDetailsScreen({ navigation, route }: Props) {
  ...
  const fetchOrder = React.useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:4000/api/v1/orders/admin/${route.params.orderId}`);
      if (res.ok) {
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    try {
      ...
      const res = await fetch(`http://localhost:4000/api/v1/orders/admin/${route.params.orderId}/status`, {
        method: 'PUT',
