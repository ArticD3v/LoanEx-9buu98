...
import { MOCK_EMI_APPLICATIONS } from '../data/mockData';
...
import { findFiCaseForApplication } from '../../fi/utils/findFiCaseForApplication';
...
export function EmiApplicationDetailsScreen({ navigation, route }: Props) {
  const application = MOCK_EMI_APPLICATIONS.find(
    (item) => item.id === route.params.applicationId,
  );

  const [fiCase, setFiCase] = useState<FiCase | undefined>(() =>
    application ? findFiCaseForApplication(application) : undefined,
  );
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      setFiCase(findFiCaseForApplication(application));
    }, [application]),
  );
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <Card style={styles.section}>
          ...
          <DetailRow label="Status" value={STATUS_LABEL[application.status]} isLast />
        </Card>
