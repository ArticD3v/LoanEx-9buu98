...
export function FiCaseDetailsScreen({ navigation, route }: Props) {
  ...
  const [fiStatus, setFiStatus] = useState<FiCaseStatus>(
    () => getFiCaseById(fiCaseId)?.status ?? 'pending',
  );
  const [draft, setDraft] = useState<FiWorkflowDraft>(() => getFiWorkflowDraft(fiCaseId));
  ...
  const handleSaveDraft = () => {
    ...
    Alert.alert('Draft Saved', 'Field Investigation draft saved (UI only).');
  };
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            if (latest) setFiStatus(latest.status);
            setDraft(getFiWorkflowDraft(fiCaseId));
            Alert.alert('Submitted', 'FI status updated to Completed (mock).');
          },
        },
      ],
    );
