...
export function UserDetailsScreen({ navigation, route }: Props) {
  ...
  useSyncExternalStore(subscribeUsers, getUsers, getUsers);
  const user = getUserById(userId);
  const master = useMasterData();

  const [branches, setBranches] = useState<string[]>(() => user?.branches ?? []);
  const [pincodes, setPincodes] = useState<string[]>(() => user?.pincodes ?? []);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (latest) {
        ...
        setPincodes([...latest.pincodes]);
      }
    }, [userId]),
  );

  const loginHistory = useMemo(() => getLoginHistory(userId), [userId]);
  const activityLog = useMemo(() => getActivityLog(userId), [userId]);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            setUserBlocked(user.id, nextBlocked);
            Alert.alert(
              'Updated',
              nextBlocked ? 'User blocked (UI only).' : 'User unblocked (UI only).',
            );
          },
        },
      ],
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () =>
            Alert.alert(
              'Reset Link Sent',
              `Password reset link sent to ${user.email} (mock — no backend).`,
            ),
        },
      ],
    );
  ...
  const handleSaveAccess = () => {
    updateUserAccess(user.id, { branches, pincodes });
    Alert.alert('Access Updated', 'Branch and pincode mapping saved (UI only).');
  };
