...
import { PAYMENT_TERMS, SETTLEMENT_CYCLES } from '../../modules/products/data/mockData';
  ...
  return (
    <View>
      ...
      <Dropdown
        ...
        onSelect={(paymentTerms) => onChange({ ...value, paymentTerms })}
      />
      <Dropdown
        label="Settlement Cycle"
        placeholder="Select settlement cycle"
        value={value.settlementCycle || ''}
        options={SETTLEMENT_CYCLES}
        onSelect={(settlementCycle) => onChange({ ...value, settlementCycle })}
      />
      <StatusToggle status={value.status} onChange={(status) => onChange({ ...value, status })} />
    </View>
  );
  ...
  return (
    <View>
      ...
      <Input
        ...
        value={value.dealerBranch || ''}
        onChangeText={(dealerBranch) => onChange({ ...value, dealerBranch })}
      />
      <Input
        label="Contact Person"
        placeholder="Contact person name"
      ...
      <Input
        ...
        autoCapitalize="characters"
      />
      <FieldLabel>KYC Document</FieldLabel>
      <ImagePlaceholder
        size="md"
        label="KYC Doc"
        subtitle="Tap to upload (mock)"
        imageUri={value.kycDocumentUri || null}
        onPress={() =>
          onChange({
            ...value,
            kycDocumentUri: mockUploadUri(value.name || 'kyc'),
          })
        }
        style={styles.image}
      />
      <Input
        ...
        onChangeText={(bankAccountNumber) => onChange({ ...value, bankAccountNumber })}
      ...
      <Input
        ...
        onChangeText={(ifscCode) => onChange({ ...value, ifscCode })}
        autoCapitalize="characters"
      />
      <Dropdown
        label="Payment / Settlement Terms"
        placeholder="Select terms"
        value={value.paymentSettlementTerms || ''}
        options={[...PAYMENT_TERMS, ...SETTLEMENT_CYCLES]}
        onSelect={(paymentSettlementTerms) =>
          onChange({ ...value, paymentSettlementTerms })
        }
      />
      <StatusToggle status={value.status} onChange={(status) => onChange({ ...value, status })} />
    </View>
    ...
    <View>
      ...
      <Input
        ...
        onChangeText={(v) =>
          onChange({
            ...value,
            capacity: v.trim() === '' ? undefined : Number(v) || 0,
          })
        }
        keyboardType="numeric"
        hint="Units — for future stock-capacity tracking"
      />
      <StatusToggle status={value.status} onChange={(status) => onChange({ ...value, status })} />
    </View>
  );
